...
const int MAX_MAINTAINS = 10; // Change later
// Modifiy these data sets  for what depth to maintain
// Should grab data from around 2.5m
const float MIN_MAINTAIN_DEPTH = 2;
const float MAX_MAINTAIN_DEPTH = 3;
...
const unsigned long DATA_SEND_DELAY = 25000; // 3 sec data send delay
...
const unsigned long PSI_HALF_OFFSET = 1500;
const long LIST_UPDATER_INTERVAL = 5000;
const unsigned long SEND_INTERVAL = 1700; // 1 second between data points
...
const unsigned long INTERVAL_STOP_INTERVAL = 0; // Every _ milisecs it'll stay off
...
void setup() {
  ...
  rf95.setModemConfig(RH_RF95::Bw500Cr45Sf128); // <--- ADD THIS LINE HERE
  ...
  if (send_float) {
    ...
    if (has_maintained) {
      if (millis() - last_send_time >= SEND_INTERVAL) {
       ...
       if (depth < 0.2) {     //may need to look at
          //digitalWrite(LED_BUILTIN, LOW); // blink every 500ms
          if (!first_send) {
...
void maintainDepth(int total_maintain_updates, float curr_depth) {
  ...
  for (int i = 0; i < depthList.getSize(); i++) {
    ...
    if (curr_depth < MIN_MAINTAIN_DEPTH && !(psi_change >= .05)) {
      if (!switch_top_state) {
        motor_direction = COUNTERCLOCKWISE;
      } else {
        motor_direction = STALLED;
      }
    } else if (curr_depth > MAX_MAINTAIN_DEPTH && !(psi_change <= -.05)) {
      if (!switch_bottom_state) {
        motor_direction = CLOCKWISE;
      } else {
        motor_direction = STALLED;
      }
    } else if (psi_change <= -.05 && curr_depth > MAX_MAINTAIN_DEPTH  ) {
        motor_direction = STALLED;
    }
    else if (psi_change >= .05 && curr_depth < MIN_MAINTAIN_DEPTH) {
        motor_direction = STALLED;
    }
    else {
      motor_direction = STALLED;
