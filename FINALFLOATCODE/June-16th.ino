...
const int MAX_LIST_SIZE = 750;
const String COMPANY_NAME = "RN18";
...
const unsigned long DATA_SEND_DELAY = 10000; // 3 sec data send delay
...
const long RADIO_TASK_INTERVAL = 500;
const unsigned long PERIOD = 1000;
const unsigned long PSI_HALF_OFFSET = 2000;
const long LIST_UPDATER_INTERVAL = 5000;
const unsigned long SEND_INTERVAL = 1250; // 1 second between data points
...
const float MAX_PSI_ROC = 0.02;
const float PSI_ROC_FLIP = 0.02;
const float TOLERABLE_DEPTH = 0.03;
...
void loop() {
  ...
  float psi = ((0.0372 * pressure_volt_reading) - 3.88) - psi_surface_start;
  ...
  if(has_maintained) {
    if (digitalRead(SWITCH_BOTTOM_PIN) == HIGH) {
      //sendLoRaMessage("Switch setup with HIGH");
      motor_direction = CLOCKWISE; //push water out, go to surface 
    } else {
      motor_direction = STALLED;
    }
  }
  ...
  if (current_millis - list_updater_millis >= LIST_UPDATER_INTERVAL) {
    ...
    // Save data
    if (psiList.getSize() < MAX_LIST_SIZE && send_float) {
      psiList.add(psi);
      depthList.add(depth);
  ...
  if (strcmp(received_data, "initiate") == 0 && initiateCount == 0) {
    if(initiateCount == 0) {
      ...
      delay(1000);
      sendLoRaMessage(" | " + COMPANY_NAME + " | Time: " + String(current_millis) + "ms |" +" | Depth: " + String(depth, 2) + "m |");
      is_initial_dive = true; // Make sure this is here
  ...
  if (send_float) {
    ...
    if (has_maintained) {
      if (millis() - last_send_time >= SEND_INTERVAL) {
       ...
       if (depth < 0.05 && psi_change < .1) {     //may need to look at
          //digitalWrite(LED_BUILTIN, LOW); // blink every 500ms
          if (!first_send) {
...
void maintainDepth(int total_maintain_updates, float curr_depth) {
  ...
  // This new block replaces your old motor control logic inside maintainDepth
    if (curr_depth < MIN_MAINTAIN_DEPTH) {
        // --- WE ARE TOO SHALLOW: Use Pulsed Descent ---
        if (interval_state == INTERVAL_MOVING) {
            motor_direction = COUNTERCLOCKWISE; // Descend
            if (millis() - interval_timer > INTERVAL_GO_INTERVAL) {
                interval_state = INTERVAL_PAUSING;
                interval_timer = millis();
            }
        } else { // state is INTERVAL_PAUSING
            motor_direction = STALLED;
            if (millis() - interval_timer > INTERVAL_STOP_INTERVAL) {
                interval_state = INTERVAL_MOVING;
                interval_timer = millis();
            }
        }

    } else if (curr_depth > MAX_MAINTAIN_DEPTH) {
        // --- WE ARE TOO DEEP: Use Pulsed Ascent ---
        if (interval_state == INTERVAL_MOVING) {
            motor_direction = CLOCKWISE; // Ascend
            if (millis() - interval_timer > INTERVAL_GO_INTERVAL) {
                interval_state = INTERVAL_PAUSING;
                interval_timer = millis();
            }
        ...
        // Reset the interval logic so it's ready for the next time we drift out of the zone.
        interval_state = INTERVAL_MOVING;
        interval_timer = millis();
    }
  } else {
    ...
    if (abs(psi_change) >= PSI_ROC && curr_depth > TOLERABLE_DEPTH) {
...
void flipMotor (float psi_change, float depth) {
  ...
  */
      ...
      if(psi_change < -PSI_ROC_FLIP && depth > TOLERABLE_DEPTH) { // -PSI_ROC_FLIP || -0.5
        motor_direction = COUNTERCLOCKWISE;//bring water in 
        
      }
      else if (psi_change > PSI_ROC_FLIP && depth > TOLERABLE_DEPTH) { // PSI_ROC_FLIP || 0.5
