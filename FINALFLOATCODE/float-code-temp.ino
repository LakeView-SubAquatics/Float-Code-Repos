...
const float MIN_MAINTAIN_DEPTH = 0.95;
const float MAX_MAINTAIN_DEPTH = 1.05;
...
// Psi Rate of Change
const float PSI_ROC = 0.050;
const float MAX_PSI_ROC = 0.125;
...
void loop() {
  ...
  if (send_float) {
    ...
    case INTERVAL_MOVING:
      checkMotorState(float_curr_state, depth, abs(psi_change), switch_top_state, switch_bottom_state);
      moveMotor(motor_direction);
      if (current_millis - interval_timer >= INTERVAL_GO_INTERVAL) {
        interval_state = INTERVAL_PAUSING;
        interval_timer = current_millis;
        //printPSI();
      }
      break;
    case INTERVAL_PAUSING:
      motor_direction = STALLED;
      moveMotor(motor_direction);
      if (current_millis - interval_timer >= INTERVAL_STOP_INTERVAL) {
...
void flipMotor (float psi_change) {
  /*switch (motor_direction) {
    case CLOCKWISE:
      motor_direction = COUNTERCLOCKWISE;
      break;
